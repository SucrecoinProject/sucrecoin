...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::XSR);
    ...
    if (!settings.contains("nAnonymizeXSRAmount"))
        settings.setValue("nAnonymizeXSRAmount", 1000);

    nAnonymizeXSRAmount = settings.value("nAnonymizeXSRAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeXSRAmount"))
        SoftSetArg("-anonymizesucrecoinamount", settings.value("nAnonymizeXSRAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in sucrecoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeXSRAmount:
            return QVariant(nAnonymizeXSRAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeXSRAmount:
            nAnonymizeXSRAmount = value.toInt();
            settings.setValue("nAnonymizeXSRAmount", nAnonymizeXSRAmount);
            emit anonymizeXSRAmountChanged(nAnonymizeXSRAmount);
            break;
        case CoinControlFeatures:
