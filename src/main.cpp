...
#if defined(NDEBUG)
#error "XSR cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 20 * 60;
int64_t nReserveBalance = 0;

/** Fees smaller than this (in usucrecoin) are considered zero fee (for relaying and mining)
...
bool RecalculateXSRSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // XSR: We use certain sporks during IBD, so check to see if they are
